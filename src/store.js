  ...
  state: {
    ...
    jobDesk: [
      ...
      },
      {
        for: 1,
        detail:
          "Collaborate with cross-functional teams, create integrated dashboard applications, utilize rest API as data exchange, use Figma as an application design tool.",
      },
    ],
  },
  getters: {
    getPortfolioItems: (state) => state.portfolioItems,
    getPortfolioItemById: (state) => (id) =>
      state.portfolioItems.find((item) => item.id === parseInt(id)),
    getTools: (state) => state.tools,
    getToolsById: (state) => (id) =>
      state.tools.find((item) => item.id === parseInt(id)),
    getEducations: (state) => state.educations,
    getEducationsById: (state) => (id) =>
      state.educations.find((item) => item.id === parseInt(id)),
    getWorkExperiences: (state) => state.workExperiences,
    getJobDesks: (state) => state.workExperiences,
    getJobDesksByWorkId: (state) => (id) =>
      state.jobDesk.filter((job) => job.for === id),
  },
});
